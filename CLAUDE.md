...
17. **`actualAward` / `totalAwd` formula**: Must sum `total_awarded` for **ALL** WPs (`filtWPs.reduce(...)`), NOT just those with `award_status === 'Awarded'`. Filtering to only Awarded WPs excludes Partially Awarded amounts and produces wrong totals. In `project.html` `buildMetrics()`: `wps.reduce(...)`. In `index.html` `renderAll()`: `filtWPs.reduce(...)`.
