...
**PWA / Add-to-Home-Screen icon**: `manifest.webmanifest` (repo root) + `assets/icons/` (generated from `../procurement app-logo.png` via Pillow â€” red Megawide mark + "PROCUREMENT APP" on a **white** square bg so the black text stays visible; iOS would otherwise fill transparency with black). Icons: `apple-touch-icon.png` (180, iOS), `icon-192/512.png` (`purpose:any`), `icon-192/512-maskable.png` (18% padding for Android adaptive safe-zone), `favicon-32.png`. **Every** `*.html` `<head>` carries the same block (`rel=manifest`, `apple-touch-icon`, favicon, `apple-mobile-web-app-*` + `mobile-web-app-capable` metas, title "Procurement") â€” inserted idempotently (skip if `apple-touch-icon` already present). All paths are relative (pages + assets are at the `/prc-app/` root). To regenerate icons: re-run the Pillow snippet (white bg, center, pad 6% regular / 18% maskable).
