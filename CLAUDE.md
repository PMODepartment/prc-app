...
A **self-contained** tool (open to any **approved** signed-in user — the admin-only gate was removed; **contributors can parse/preview and import to an empty project**, since RLS lets them INSERT WPs on assigned projects. Only the **Replace** step — deleting an existing project's WPs before re-inserting — is super-admin-gated by RLS, so a contributor's Replace aborts at the delete safety-net. The "open the Excel importer →" links in `wp-form.html` + the `project.html` CSV modal are labelled *"(replacing an existing project's WPs requires super-admin)"* — the earlier *"(admin only)"* labels were stale and wrongly discouraged contributors) that reads a raw **Work-Package Monitoring `.xlsx` directly in the browser** (SheetJS via CDN) — no Python extractor, no CSV step. It **replaces** the old `gen_import.py` + generated `avr101-import.html` workflow (both git-ignored — `avr101-import.html` embeds real data and must never be committed). Loads supabase-js + SheetJS via CDN, reuses the existing localStorage session. Stores **digit-only `wp_no`** (no `WP-` prefix). **Delete-permission safety net:** after the "Replace" delete it re-counts the project's WPs and aborts before inserting if any remain (RLS restricts WP delete to `super_admin`, so a blocked delete can't silently create duplicates).
