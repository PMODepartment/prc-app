...
- **Dark mode**: Toggle button in topbar (`renderUserBar` in `db.js`). `AppTheme` IIFE in `ui.js` — `init(userId)`, `toggle(userId)`, `apply(dark)`. Preference stored in `localStorage`: `wpm_theme_{userId}` (per-user) + `wpm_theme_last` (fast anti-flash). Anti-flash IIFE at top of `ui.js` applies `dark-mode` class to `body` and `html` before render. `auth.js` calls `AppTheme.init(userId)` after profile load. Charts updated via `Charts.updateTheme(dark)` in `charts.js`. **Dark mode CSS pitfall**: inline `<style>` blocks in `project.html` and `index.html` define `.budget-table`, `.ov-donut-card`, `.ov-stats-card`, `.works-kpi`, `.sched-wp` etc. with hardcoded light colors — these override cascade and are NOT caught by attribute selectors (`[style*="color:#231F20"]`). Overrides for these class rules live in the `/* ── Tables ── */` section of `dashboard.css` under `body.dark-mode .budget-table td` etc. Also: inline `background:#fafafa` (total rows, trade group headers) and `background:#f8f8f8` (sticky WP table headers, sub-tab container) are covered by attribute selectors in `dashboard.css`.
