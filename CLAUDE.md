...
### Save Validation
Required fields (in `save()`): WP No., Work Package Description, Project, Trade, Awarding Date, Target Delivery Date, Target Completion Date, Approved Budget, Charging type. **Cost Code No. is NOT required** (label has no `*`, no validation check). **Actual Awarding Date cannot be in the future** — if `f-actual-awarding-date` parses to a date after today (local midnight), `save()` aborts with "Actual Awarding Date cannot be in the future." (a WP can't be awarded on a date that hasn't happened yet). The **CSV import** path enforces the same rule: both `importWPsFromCSV()` (in `project.html` and `wp-form.html`) validate the whole batch up front and `throw` (naming the offending WP No(s)) before any DB write, so a future `actual_awarding_date` aborts the entire import with no partial rows.
...
17. **`actualAward` / awarded count come from `helpingMetrics(wps)` in `db.js` — now ROW-DERIVED by default** (was Helping-Sheet-driven). The default branch derives each project's **Awarded count, Total Budget (BCB), Budget-for-Awarded, and Awarded Cost (= "Actual Award")** directly from the imported per-WP rows: a WP is **awarded when `total_awarded>0`**, and awarded count, awardedCost AND budgetAwarded all use that SAME set so they reconcile with each other and with the chart/forecast (which also key off `total_awarded>0`). This is self-maintaining for all NEW projects. **`HELPING_FIGURES` is now only a transitional OVERRIDE** for legacy projects whose rows we haven't reconciled yet (GPR101/OPW101/SLN101/SLT101/UTM101). **Why the Helping Sheet can be wrong:** it builds Awarded Cost from a date-gated cumulative (`SUMIFS` on Actual Award Date over a *hardcoded* row range), so it silently DROPS awarded WPs that have a **blank actual-award date** or sit **below the formula's last row**. AVR101 was exactly this (Helping Sheet ₱257.19M vs true per-WP ₱266.11M — understated ₱8.9M; it actually runs ~₱10M OVER budget), so **AVR101 was removed from `HELPING_FIGURES`** to use the accurate row-derived values. Use `project.html?id=<PID>&diag=1` to see each project's row-vs-Helping gap, then remove it from the table once trusted. UTM101 has the opposite gap (Helping ₱1.86B vs per-WP ₱1.34B) — keep its override pending investigation. (Old bug, now fixed: the fallback summed `total_awarded` over ALL rows but budget over only `award_status='Awarded'` rows — an inconsistent mix.) Consumers: `buildMetrics()`/`renderOvBattery()`/`renderStatusTable()` (project.html); `renderAll()`/`renderIdxOvBattery()`/`computeStats()` (index.html). `Cost to Complete = totalBudget − budgetAwarded`; `Variance = totalBudget − (awardedCost + CTC)`. Due-Not-Awarded / Not-Due splits remain row+date derived.
18. **`procurement_status` vs `award_status` (DECOUPLED)**: Dashboard KPIs (`buildMetrics`, `computeStats`) read only `award_status` (`Not Yet Awarded | Partially Awarded | Awarded`). `procurement_status` (`Not Started | Sourcing | Solicitation | Evaluation & Negotiation | Awarded` — updated 2026-06) is an **independent** stage field; even though it now includes an "Awarded" stage value, award is still determined solely by `award_status` (the two do not sync). `onProcStatusChange()` is a no-op (just `markDirty`); the two fields no longer sync. Award is determined solely by Award Status, which on import comes from the WP-Monitoring **Remarks (Col P)** "Not yet Awarded / Awarded" text.
