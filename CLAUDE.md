...
7. **Trade name consistency — 10 canonical trades only**: `normTrade(t)` in `index.html` and `project.html` (identical) collapses EVERY trade string into one of exactly 10: General Requirements, Site Works, Structural Works, Architectural Works, Mechanical Works, **Electrical Works**, Plumbing Works, Fire Protection Works, Allied Services, Site Development Works. **Electrical Works and Auxiliary Works are combined into a single "Electrical Works" bucket** (per user request, for simplicity): `electrical works` → Electrical Works; `auxiliary works` → Electrical Works; the combined `electrical and auxiliary works` / `electrical & auxiliary works` → Electrical Works. Other variants: OPW101 granular arch sub-trades → Architectural; Structural Labor And Services → Structural; Housekeeping & Sanitation / Drawing / Security Services → General Requirements; Other Allied Services → Allied Services. Keyword-regex fallback: both `/electr/` AND `/auxiliary/` → Electrical Works. Applied at data load time. `IDX_TRADE_ORDER`/`TRADE_ORDER` list the same 10 (drives chart colour order; "Auxiliary Works" removed). Keep the explicit map + keyword fallback in sync across both files.
