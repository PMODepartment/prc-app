...
- **`XL_COLS`** — 33 column defs `{k,label,w,type,opts,ro}`; `type` in `text|num|money|date|select` drives both the formatter and the editor. Selects carry the canonical option sets (`XL_TRADES` 10 trades, `XL_PROC`, `XL_SUBM`, `XL_DELIV`, Yes/No, Main Contract/Change Order) — **keep these in sync with the WP-form dropdowns and the Status Tracker**. A stored value outside the set is prepended so legacy data still shows.
