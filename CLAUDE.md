...
- **Layout profiles (`LAYOUTS`), auto-detected + selectable** — the monitoring sheets differ per project, so each layout is a named column-map profile picked via a **Sheet layout** dropdown (auto-selected from the sheet/file name):
  - **`standard`** (AVR / SLN / SLT / GRP / UTM, ported from `gen_import.py`): `wp_no` col 4, budget col 30, awarded col 31, remarks col 146, stage cols 127–133, dates 119/120/122/124/125; data starts after **row 17**; `trade = type_of_service` (col 9); award inferred from awarded cost.
  - **`opw`** (One Portwood, `OPW WP Monitoring` sheet): **VAT-EX** cost columns (budget col 31, awarded col 34, additionals col 35), `wp_no` col 3, explicit **award-status text** in col 13, schedule cols 136–144, stage cols 146–152, remarks col 175; data starts after **row 7**. **Hierarchical trade:** the trade comes from the ALL-CAPS group-header rows (`GENERAL REQUIREMENTS` / `SITE WORKS` / `STRUCTURAL WORKS` / `ARCHITECTURAL WORKS` / `MEPF WORKS` / `OTHER ALLIED SERVICES`) tracked while scanning (SUBTOTAL/GRANDTOTAL ignored); **`MEPF WORKS` is split** into Mechanical / Electrical and Auxiliary / Plumbing / Fire Protection by keyword on the WP description (ambiguous → left as `MEPF WORKS` for review). Verified against the real OPW file (104 WPs across 6 trades).
  - `rowToWP(r, pid, today, L, curTrade)` is layout-driven with a null-safe column getter; award prefers `L.awardTextCol` (OPW col 13) then falls back to awarded-cost.
- **Sheet + project pickers:** the workbook's sheet list (defaults to `/monitoring/i`) and a target-project dropdown (**any project**).
