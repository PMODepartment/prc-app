...
### Save Validation
Required fields (in `save()`): WP No., Work Package Description, Project, Trade, Awarding Date, Target Delivery Date, Target Completion Date, Budget (BCB), Charging type. **Cost Code No. is NOT required** (label has no `*`, no validation check). **Award-status safeguards:** when Award Status = `Awarded`, both **Awarded Cost (Net)** (`f-contract`) and **Awarded Vendor** (`f-contractor`) are required. **PO/JO consistency:** `No. of PO/JO` (`f-po-jo-count`) must equal the number of entries in `PO/JO Numbers` (`f-po-jo-numbers`, split on `/[\s,;]+/`); mismatch aborts the save. **Actual Awarding Date cannot be in the future** — if `f-actual-awarding-date` parses to a date after today (local midnight), `save()` aborts with "Actual Awarding Date cannot be in the future." (a WP can't be awarded on a date that hasn't happened yet). The **CSV import** path enforces the same rule: both `importWPsFromCSV()` (in `project.html` and `wp-form.html`) validate the whole batch up front and `throw` (naming the offending WP No(s)) before any DB write, so a future `actual_awarding_date` aborts the entire import with no partial rows.
