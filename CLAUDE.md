...
- **`WPCsv.toWPData(table, pid)`** — header-name mapping; falls back to position-based (native 54-col / legacy <40-col) when no header match. Handles: **Excel serial-number dates** (`pDate` converts e.g. `46050` → `2026-01-28` via the 1899-12-30 epoch; range 20000–80000); **stage-derived procurement_status** — the 7 WP-Monitoring stage columns (`SOURCING, RFQ, BID OPEN, BID CLOSED, LOA, CONTRACT, MOB / DEL`) are read and the **right-most marked stage** (X/TRUE/date) becomes `procurement_status`; **award_status inference** — explicit "AWARDED" text, else Contract/Mob-Del stage ⇒ Awarded, else awarded_cost>0 ⇒ Awarded; `pPct` treats values >1 as percentages (÷100) and ≤1 as already-decimal. Arbitrary `type_of_works` (e.g. "Materials" only) and combined trades (e.g. "Electrical & Auxiliary") pass through unchanged — dashboards group by exact trade string.
- **`WPCsv.dataRowCount(table)`** — counts rows with a WP No. for the "Ready to import N" message.

> Verified against `EPC. PMO. Import WP. SLN101. 2025 06 11.csv`: 87 WPs, dates converted, statuses derived. The `f-proc-status` dropdown in `wp-form.html` now offers the WP-Monitoring stages (Not Started / Sourcing / RFQ / Bid Open / Bid Closed / LOA / Contract / Mob / Del / Awarded); `onProcStatusChange()` sets award_status=Awarded for Contract/Mob-Del/Awarded.

The native template (`downloadCSVTemplate`) remains 54-column. Legacy files (<40 cols, no header match) still parse via position fallback.
