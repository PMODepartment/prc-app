...
**WP List** (`index.html`): `_WPC` column defs + `_WP_VIEWS` + `_getActiveCols()`. `renderWPMonTable()` rebuilds colgroup/thead per view and also populates `#wp-mon-cards` (mobile card view). `setWPListView(view)` switches + resets sort. `openWPDetail(w)` / `closeWPDetailModal()` â€” slide-in panel. All tab hidden for viewers.
  ...
  - **Year + Month/Quarter scope** (`wpmon-year` + `wpmon-period-sel`, state `_wpMonYear`/`_wpMonPeriodSel`): two dropdowns in the Filtered Analysis header restrict the **KPIs + chart + table** to WPs whose **planned award date (`awarding_date`)** falls in the chosen year and month/quarter (`_wpMonPeriodMatch`). Both "all" = unrestricted (keeps null-date rows). The period dropdown lists Months in Monthly mode / Quarters in Quarterly mode (`fillWpMonPeriodOptions`, refilled by `setWpMonPeriodMode`; resets to "all" when the mode switch makes the current value incompatible). Year options come from the data (`populateWpMonAnalysisFilters`, called from `populateWPMonTradeFilter` on load). Scope applies ONLY to the analysis panel, not the main WP list below.
