...
A **self-contained** tool (open to any **approved** signed-in user — the admin-only gate was removed) that reads a raw **Work-Package Monitoring `.xlsx` directly in the browser** (SheetJS via CDN) — no Python extractor, no CSV step. It **replaces** the old `gen_import.py` + generated `avr101-import.html` workflow (both git-ignored — `avr101-import.html` embeds real data and must never be committed). Loads supabase-js + SheetJS via CDN, reuses the existing localStorage session. Stores **digit-only `wp_no`** (no `WP-` prefix). **Delete-permission safety net:** after the "Replace" delete it re-counts the project's WPs and aborts before inserting if any remain (RLS restricts WP delete to `super_admin`, so a blocked delete can't silently create duplicates).
