...
- **Dark mode**: Toggle button in topbar (`renderUserBar` in `db.js`). `AppTheme` IIFE in `ui.js` — `init(userId)`, `toggle(userId)`, `apply(dark)`. Preference stored in `localStorage`: `wpm_theme_{userId}` (per-user) + `wpm_theme_last` (fast anti-flash). Anti-flash IIFE at top of `ui.js` applies `dark-mode` class to `body` and `html` before render. `auth.js` calls `AppTheme.init(userId)` after profile load. Charts updated via `Charts.updateTheme(dark)` in `charts.js`.
  ...
  - **`login.html` dark mode (self-contained)**: login/register/etc. don't load `ui.js`/`dashboard.css`, so login has its OWN dark mode — inline anti-flash `<script>` in `<head>` adds `dark-mode` to `<html>` from `wpm_theme_last` (or system pref if unset); CSS rules are prefixed `html.dark-mode` (set early enough to avoid a white flash on `.form-wrap`); a fixed top-right `.login-theme-toggle` (sun/moon inline SVG) calls `toggleLoginTheme()` which persists `wpm_theme_last`. Shares the `wpm_theme_last` key with the dashboard so the theme carries across login→dashboard. (register/pending/forgot-password are NOT yet themed — replicate this pattern if needed.)
