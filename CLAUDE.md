...
7. **Trade name consistency — 11 canonical trades only**: `normTrade(t)` in `index.html` and `project.html` (identical) collapses EVERY trade string into one of exactly 11: General Requirements, Site Works, Structural Works, Architectural Works, Mechanical Works, **Electrical Works**, **Auxiliary Works**, Plumbing Works, Fire Protection Works, Allied Services, Site Development Works. `electrical works` → Electrical Works; the combined `electrical and auxiliary works` / `electrical & auxiliary works` → **Electrical Works** (electrical is primary; can't split a single label per-WP); pure `auxiliary works` → Auxiliary Works. Other variants: OPW101 granular arch sub-trades → Architectural; Structural Labor And Services → Structural; Housekeeping & Sanitation / Drawing / Security Services → General Requirements; Other Allied Services → Allied Services. Keyword-regex fallback checks `/electr/`→Electrical before `/auxiliary/`→Auxiliary. Applied at data load time. `IDX_TRADE_ORDER`/`TRADE_ORDER` list the same 11 (drives chart colour order). Keep the explicit map + keyword fallback in sync across both files.
