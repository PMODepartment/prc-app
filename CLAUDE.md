...
WPDb.deleteUser(id)                        // hard-delete a user's public.users profile row (admin "Remove")
...
**Admin "Remove" user (hard delete):** the old soft "Revoke" (set `status='rejected'`) is now **"Remove"** — `admin.html` `revokeUser` calls `WPDb.deleteUser(id)` which hard-deletes the `public.users` profile row (strong confirm first). The user loses access immediately (no profile → `AppAuth.requireLogin` signs them out) and would have to register again. NOTE: the underlying `auth.users` record can only be purged with the service-role key server-side, but without a profile they can't sign in. The admin KPI/badge label "Revoked" → **"Rejected"** (only denied *registrations* remain in `status='rejected'` now).
