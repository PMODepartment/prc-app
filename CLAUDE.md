...
20. **Never commit real procurement data**: `data/*.json` real WP dumps were removed (they were publicly served via GitHub Pages, bypassing auth/RLS). `data/` is now git-ignored. Git history still contains them — a history scrub (`git filter-repo`) or private-repo migration is the real remedy. The legacy `assets/js/data.js` static loader was deleted (unused).
