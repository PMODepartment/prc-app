...
| Dashboard | Period chart + **WP Status split cards** (donut card with % by WP/Value toggle + stats table card) + WP by Trade bar + backlog table + Top 5 panels |
...
- **Dashboard**: Period chart + **WP Status split** (donut card `c-dash-status` with `setDashStatusMode('wp'|'val')` toggle + stats table `#dash-status-table`) + WP by Trade + backlog table + Top 5 panels
